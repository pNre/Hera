...
FaceDetector::FaceDetector(
    ...
    const char *weights_path) : confidence_threshold_(0.5), input_image_height_(300), input_image_width_(300),
                                scale_factor_(1.0), mean_values_({104., 177.0, 123.0})
{
    ...
    network_ = cv::dnn::readNetFromCaffe(config_path, weights_path);
    ...
    if (network_.empty())
    {
        ...
        ss << "Failed to load network with the following settings:\n"
           ...
           << "Binary: " + std::string(weights_path) + "\n";
