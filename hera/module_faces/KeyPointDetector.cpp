...
KeyPointDetector::KeyPointDetector(const char *model_path) {
    facemark_ = cv::face::FacemarkLBF::create();
    facemark_->loadModel(model_path);
}

std::vector<std::vector<cv::Point2f>> KeyPointDetector::detect_key_points(const std::vector<cv::Rect> &face_rectangles, const cv::Mat &image) const {
