...
let host_and_port =
  Host_and_port.{ host = "api.telegram.org"; port = 443 }

let post path body =
  let req = Http.{ host_and_port; http_method = `POST; http_headers = []; path } in
  Http.request req ~on_write_body:(fun request_body ->
      Body.write_string request_body body;
      Body.close_writer request_body) ()

let get path =
  Http.request { host_and_port; http_method = `GET; http_headers = []; path } ()
