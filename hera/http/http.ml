...
let request req ?(on_write_body = Body.close_writer) () =
  Log.Global.info "Requesting %s" req.path;
  let where_to_connect = Tcp.Where_to_connect.of_host_and_port req.host_and_port in
  Tcp.connect_sock where_to_connect
  >>= (fun socket ->
      let request = Request.create ~headers:(request_headers req.host_and_port.host req.http_headers)
          req.http_method
          req.path in
      Deferred.create (fun i ->
          let request_body = Client.SSL.request
